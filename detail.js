...
function getItemById(id) {
  return readingListData.find(item => item.id === id);
}

function getNeighbors(index) {
  return {
    prev: index > 0 ? readingListData[index - 1] : null,
    next: index < readingListData.length - 1 ? readingListData[index + 1] : null
  };
}
...
function renderMissing() {
  detailRoot.innerHTML = `
    <section class="card detail-card">
      <p class="eyebrow">Reading detail</p>
      <h1 class="detail-title">Item not found</h1>
      <p class="subtitle">The reading item you're looking for is not in this tracker.</p>
    </section>
  `;
}
    ...
    <aside>
      <section class="card summary-card">
        <div class="card-header">
          <h2>Memory refresh summary</h2>
          <p>A short orientation to the work's main ideas</p>
        </div>
      ...
      <section class="card nav-card" style="margin-top:18px;">
        <div class="card-header">
          <h2>Navigate</h2>
          <p>Move through the reading list in order</p>
        </div>
        <div class="nav-list">
          ${neighbors.prev ? `
            <a class="nav-button" href="detail.html?id=${encodeURIComponent(neighbors.prev.id)}">
              <small>Previous</small>
              <strong>${neighbors.prev.order}. ${neighbors.prev.title}</strong>
            </a>` : ""}
          ${neighbors.next ? `
            <a class="nav-button" href="detail.html?id=${encodeURIComponent(neighbors.next.id)}">
              <small>Next</small>
              <strong>${neighbors.next.order}. ${neighbors.next.title}</strong>
            </a>` : ""}
        </div>
      </section>
    </aside>
