...
function getVisibleItems(state) {
  ...
  return readingListData.filter(item => {
    ...
    const matchesQuery = !query || item.title.toLowerCase().includes(query) || item.author.toLowerCase().includes(query) || item.category.toLowerCase().includes(query) || notes.includes(query);
...
function updateItemState(id, data) {
  const state = loadState();
  state.items[id] = { status: data.status || "not-started", done: data.status === "done", notes: data.notes || "" };
  saveState(state);
  updateSummary();
}
...
function clearChecks() {
  ...
  readingListData.forEach(item => {
    const prev = state.items[item.id] || {};
    state.items[item.id] = { status: prev.status === "reading" ? "reading" : "not-started", done: false, notes: prev.notes || "" };
  });
  saveState(state);
  renderItems();
}
...
function exportProgress() {
  const state = loadState();
  const payload = { exportedAt: new Date().toISOString(), app: "Philosophy of Mind Tracker", version: 5, data: state };
...
function importProgress(file) {
  ...
  reader.onload = () => {
    try {
      ...
      const normalized = { items: importedState.items || {}, meta: importedState.meta || {} };
